...
# Adiciona a pasta raiz do projeto (um nível acima de 'Frontend') ao path do Python.
# Isso garante que o script sempre encontre a pasta 'Backend'.
...
def iniciar_processamento(lista_de_arquivos, modo_pasta=False):
    ...
    for i, arquivo in enumerate(lista_de_arquivos):
        ...
        if not dados_brutos:
            st.error(f"Falha ao extrair dados de '{filename}'. O modelo pode não ter retornado um JSON válido.")
            continue
