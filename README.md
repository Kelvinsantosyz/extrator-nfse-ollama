...
* **Para Ollama LMM (`OCR_METHOD=OLLAMA`):** Nenhuma biblioteca Python extra *além do `ollama`* (já na lista base) é necessária, mas você precisa ter o Ollama instalado e a correr no seu sistema, com os modelos LMM desejados (ex: `ollama pull llava-llama3`) já baixados.
...
3. **Ollama (Se usar OCR_METHOD=OLLAMA ou para Extração LLM):**
...
Se quiser que eu adicione o bloco da miniatura do YouTube diretamente no `README.md` do seu repositório (e gerar o commit `README.md`), diga o caminho do repositório ou forneça acesso ao ficheiro — eu posso preparar o patch de commit para você copiar/colar.
